import { Button, Checkbox, Dropdown, Flex, FloatButton, Input, Modal, Row, Typography } from 'antd'
import React, { useEffect, useState } from 'react'
import ListPage from './ListPage';
import "./style.css";
import { faSort, faTable, faPlus } from '@fortawesome/free-solid-svg-icons';
...
export default function Page({ dataPrint, data, tableKeys, newTitle, setDataPrint, searchKeys, sortKeys, mobileKeys }) {
            ...
            <span
                ...
                onClick={() => {
                    ...
                    const sortedData = [...dataPrint].sort((a, b) =>
                        a[key?.key]?.toString().localeCompare(b[key?.key]?.toString())
                    );
    ...
    const items = [
        ...
        ...tableKeys.filter(e => !e?.required).map(({ title, dataIndex }) => ({
            key: dataIndex,
            label: (
                <Checkbox
                    className='width-100'
                    ...
                    disabled={tableKeysPrint.length <= 3 && tableKeysPrint.some(e => e.key === dataIndex)}
    ...
    useEffect(() => { setTableKeysPrint(tableKeys) }, []);

    return (
        <Flex flex={1} vertical gap={"1em"} className='container'>
            <Row align={"middle"} style={{ flexFlow: "" }} justify={"space-between"}>
                {/* add new button */}
                {!isMobile && <Row style={{}}>
                    <Button onClick={() => openModal("add")} type='primary'>{newTitle}</Button>
                </Row>}
                {/* search */}
                <Row style={{}}>
                    <Search placeholder="חיפוש ..." allowClear onChange={({ target }) => {
                        setDataPrint(data?.filter((e) =>
                            searchKeys.some((key) =>
                                e[key]?.toString()?.toLowerCase()?.includes(target.value.toLowerCase())
                            )
                        )
                        );
                    }} style={{ width: 200 }} />
            ...
            <FloatButton type='primary' onClick={() => openModal("add")} icon={<FontAwesomeIcon icon={faPlus} />} />
            {/* list table */}
            <ListPage data={dataPrint} tableKeys={tableKeysPrint} openModal={openModal} mobileKeys={mobileKeys} />
            <Modal
            ...
            >
                <DynamicFormPage
                    ...
                    fields={tableKeys.filter((key) => key.editable || modalMode === "view")}
                    onSubmit={(values) => {
                        if (modalMode === "add") onAdd(values);
                        if (modalMode === "edit") onEdit(selectedItem._id, values);
                        closeModal();
                    }}
